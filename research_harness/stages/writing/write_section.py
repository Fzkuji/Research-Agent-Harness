...
def write_section(section: str, context: str, runtime: Runtime) -> str:
    """Write one section of an academic paper at a top-venue level
    (NeurIPS/ICML/ICLR). Output Markdown (inline LaTeX math where needed).

    Rules:
    - Start each subsection with WHY (motivation), then HOW (what you did).
    - Every claim needs evidence from the provided materials or a citation.
    - Use continuous paragraphs, never bullet lists.
    - Introduction: background -> problem -> existing gaps -> our approach ->
      contributions. Only describe advantages, NO technical details (Method does that).
    ...
    - Present tense for methods/results; past tense only for historical events.
    - No AI-flavor words (leverage, delve, tapestry, utilize). Simple, clear.
    ...
    preamble, no "what would you like" questions, no talk about saving files
    (the caller persists your output).
    """
