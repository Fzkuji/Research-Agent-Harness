# Vendored from academic-research-skills v3.12.0
# https://github.com/Imbad0202/academic-research-skills
# (c) Cheng-I Wu — CC BY-NC 4.0 (https://creativecommons.org/licenses/by-nc/4.0/)
# Changes: import paths adapted to research_harness.citation_gate;
# see LICENSE in this directory. Original module: scripts/openalex_client.py
...
from research_harness.citation_gate._text_similarity import (
        _BACKOFF_SECONDS,
        _MAX_RETRIES,
...
class OpenAlexClient:
    ...
    def doi_lookup_with_title_check(
        ...
        if not data:  # 404 / no record
            return None
        title = data.get("title") or ""
        if not title.strip():
            # Upstream record carries an EMPTY title (OpenAlex inherits
            # Crossref metadata): the DOI resolved — an impossible
            # cross-check must not become fabrication evidence.
            # (Harness change; see LICENSE in this directory.)
            return data
