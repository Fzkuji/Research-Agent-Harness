...
class TestOperationLog:
    """Test that log_file parameter enables persistent logging."""

    def test_log_file_created(self, tmp_dir):
        """research_agent with log_file should create the log."""
        from research_harness.main import research_agent

        log_path = os.path.join(tmp_dir, "test.log.md")
        responses = [
            _json({"stage": "writing", "reasoning": "polish", "sub_task": "polish", "done": False}),
            _json({"call": "polish_rigorous", "args": {"text": "x"}, "reasoning": "polish"}),
            "polished",
            _json({"stage_done": True, "reasoning": "done"}),
        ...
        assert "polish_rigorous" in content
        assert "DONE" in content

    def test_no_log_file_is_fine(self):
        """research_agent without log_file should not crash."""
        from research_harness.main import research_agent
