#!/usr/bin/env python3
"""Cross-platform installer for the paper-review + humanize-paper-review
skills, plus the underlying review_app + corpus.

Default behavior:
  1. Clone (or update) https://github.com/Fzkuji/Research-Agent-Harness
     into ~/.research-agent-harness
  2. Symlink (or copy on Windows w/o dev mode) the two skills into
     ~/.claude/skills/{paper-review,humanize-paper-review}
...
def main() -> int:
    ...
    print(f"  2. Verify: python -c "
          ...
          f"echo OK")
    print(f"  3. In your agent (Claude Code, opencode, ...): /paper-review "
          f"<paper.pdf> [venue=...] or /humanize-paper-review "
          f"<paper.pdf> draft=<draft.md> [venue=...]")
    return 0
